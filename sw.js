...
async function manageCacheSize(cacheName, limit) {
  ...
  if (size > limit) {
    ...
    console.log(`Cleaned cache ${cacheName}: removed ${entriesToRemove} entries`);
  }
}
...
async function networkFirst(request, cacheName) {
  ...
  try {
    ...
    console.log(`[SW] Network failed, trying cache (${cacheName}):`, request.url);
    const cached = await cache.match(request);
...
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'SKIP_WAITING') {
    // Don't automatically skip waiting to prevent page reloads
    console.log('[SW] Skip waiting message received, but not acting on it to prevent reloads');
  }
});
